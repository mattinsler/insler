...
## Declaring a service (`defineService`)

`defineService` wraps a framework `Contract` with the service's full operational
intent — the declaration the insler platform (`@insler/platform`'s `/fleet` →
`/generator` → `/reconciler` entrypoints) compiles into deployment artifacts:

```ts
import { defineService } from '@insler/service';
import { OrdersContract, BillingContract } from './contracts';

export const orders = defineService({
  name: 'orders',
  kind: 'persistent',
  contract: OrdersContract,
  needs: ['orders-db', 'valkey'],                       // logical resources, never connection strings
  calls: [{ contract: BillingContract, method: 'charge' }], // typed — a method typo is a compile error
  scale: { on: 'cpu', min: 2, max: 10 },
  isolation: 'gvisor',                                  // 'default' | 'gvisor' | 'microvm'
  expose: { http: { method: 'POST', path: '/orders', handler: 'create' } },
});
```

The result is a deeply-frozen `ServiceDef`. It keeps the live contract for the
runtime path (`Service.create`) and serializes to a static, JSON-safe view for
the generator (`toJSON()`), so the declaration must stay **statically
analyzable** — literal intent only, no env vars or computed config.

Defaults are resolved onto always-present `effective*` projections:
`effectiveScale` (the kind's scaling signal + replica floor applied),
`effectiveIsolation`, plus `needRefs` / `callRefs` / `exposeRoutes` views the
platform consumes. Validation throws at declaration time (duplicate needs/calls,
colliding routes, a `persistent` floor of 0, …).

Identity is derived, not declared: `deriveIdentity(def, env)` yields the stable,
hierarchical `environment.namespace.name` (e.g. `prod.default.orders`) that NATS
credentials, secret paths, ServiceAccounts, and telemetry attribution key on.

Put declarations in `*.service.ts` / `*.def.ts` files — that's the convention
`@insler/platform/fleet`'s scanner discovers them by.
