...
describe('ServiceClient.create()', () => {
  test('creates a throw-mode client', async () => {
    const { transport, stop } = await setupHost(
      SimpleContract,
      simpleHandlers as Record<string, (...args: unknown[]) => unknown>
    );
  ...
  test('creates a result-mode client', async () => {
    const { transport, stop } = await setupHost(
      ContractWithErrors,
      errorHandlers as Record<string, (...args: unknown[]) => unknown>
    );
...
describe('ServiceClient.withContext()', () => {
  test('creates a scoped client with pre-applied context', async () => {
    const { transport, stop } = await setupHost(
      ContractWithContext,
      contextHandlers as Record<string, (...args: unknown[]) => unknown>
    );
