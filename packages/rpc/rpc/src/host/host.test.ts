...
describe('Host.create() handler typing', () => {
  // PRD design decision #3: handlers are plain objects matching
  // `Contract.Handlers<C>`. `Host.create` must accept exactly that shape
  // cast-free, with handler parameters inferred from the contract.
  test('accepts contract-shaped handlers without casts', async () => {
    const transport = new MockTransport();
    const host = await Host.create(
      SimpleContract,
      {
  ...
  test('a separately-declared Contract.Handlers object is accepted as-is', async () => {
    const transport = new MockTransport();
    const handlers: Contract.Handlers<typeof SimpleContract> = {
      greet: async ({ name }) => ({ message: `Hello, ${name}!` }),
    };
    const host = await Host.create(SimpleContract, handlers, transport);
    await host.stop();
  });

  test('rejects handlers that do not match the contract', () => {
    const transport = new MockTransport();
    void (async () => {
      // @ts-expect-error a handler returning the wrong output shape is rejected
      await Host.create(SimpleContract, { greet: async () => ({ wrong: true }) }, transport);
      // @ts-expect-error a missing handler is rejected at the type level
      await Host.create(SimpleContract, {}, transport);
    });
  });
});
