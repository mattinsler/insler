...
import type { Propagator } from '../context/index.js';
import { createPropagator } from '../context/index.js';
import type { Contract, ContractDef, MethodDef } from '../contract/index.js';
...
export namespace Host {
  export async function create<C extends ContractDef>(
    ...
    // Handlers are contract-typed at the boundary (`Contract.Handlers<C>` —
    // PRD design decision #3); internally the host dispatches by method name
    // and revalidates everything at runtime, so it erases to a name-keyed map.
    const handlerMap = handlers as Record<string, (...args: unknown[]) => unknown>;

    for (const methodDef of contract.methodList) {
      const handler = handlerMap[methodDef.name];
      if (!handler) {
        throw new Error(
