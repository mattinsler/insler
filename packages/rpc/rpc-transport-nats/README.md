...
## Development inner loop (leaf node)

`startLeafNode` runs a local `nats-server` **leaf node** that solicits a route to
a shared development cluster, so one or two services running locally on Bun join
the *same queue groups* as the remote fleet — participating in the real mesh
without running the whole fleet locally:

```ts
import { startLeafNode } from '@insler/rpc-transport-nats';

const leaf = await startLeafNode({
  remotes: [{
    url: 'nats-leaf://dev-cluster.example.com:7422', // the cluster's leafnode listener
    credentials: '/abs/path/dev.creds',              // resolved by tooling — never embedded in a service
  }],
});

// point local services at the leaf's local client listener:
// nats://127.0.0.1:<leaf.port>

await leaf.stop();
```

`buildLeafNodeServerConfig` / `renderLeafNodeServerConfig` expose the underlying
config generation separately. Core NATS only — no JetStream — like the rest of
this package.

One behavioral note: NATS leaf nodes prefer a locally available queue-group
member, so requests are not round-robined across the leaf/cluster boundary when
members exist on both sides. The dev loop relies on the local instance being the
only member for its subject, with failover to the remote fleet when it leaves.
