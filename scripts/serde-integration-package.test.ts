...
// Repo-level invariants for the serde subsystem's private integration package
// (subsystem-branding issue 0008, replicating the issue 0005 template and the
// di replication): it joins the workspace as a non-publishable package
// (ignored by the publish pipeline, changesets, the tsdown build, and the
// umbrella weight-invariant test), consumes the subsystem exactly as an
...
// (non-public) import fails lint. serde is a pure serialization library with
// no external infrastructure, so like the di template the suite runs entirely
// in-process against the built public surface.

const repoRoot = new URL('..', import.meta.url).pathname;
const INTEGRATION_DIR = 'packages/serde/serde-integration';
const pkg = await Bun.file(join(repoRoot, INTEGRATION_DIR, 'package.json')).json();

// serde's public surface: the single-entrypoint umbrella (the Serde
// interface) plus its four format adapter packages — derived from the
// subsystem directory so the allowlist cannot drift from what is published.
const surface = await discoverSubsystemSurface(repoRoot, 'serde');
const PUBLIC_SERDE_SURFACE = [surface.umbrellaName, ...surface.adapterPackages];
...
describe('workspace membership', () => {
  ...
  test('directory equals unscoped npm name (ADR-0003), under the serde subsystem directory', () => {
    expect(pkg.name).toBe('@insler/serde-integration');
...
describe('consumer-grade dependencies', () => {
  test('every @insler dependency is a public consumer-facing package via workspace:*', () => {
    ...
    for (const [name, range] of internal) {
      expect(PUBLIC_SERDE_SURFACE).toContain(name);
      expect(range).toBe('workspace:*');
    }
  });

  test('depends on the umbrella and every adapter (the whole public surface under test)', () => {
    for (const name of PUBLIC_SERDE_SURFACE) {
  ...
  test('needs no real infrastructure: no infra dependencies, no server harness', async () => {
    ...
    // (subsystem-branding PRD): serde is in-process serialization, so its
    // integration suite provisions nothing — no infra client deps, no
...
describe('package-boundary lint guard', () => {
  ...
  test('a relative import escaping the package fails lint (executable boundary guard)', async () => {
    ...
    await Bun.write(fixture, "import '../../serde/src/serde.js';\n");
    try {
