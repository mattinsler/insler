...
const astroConfig = await Bun.file(join(siteRoot, 'astro.config.ts')).text();
...
describe('workspace membership', () => {
  ...
  test('directory equals unscoped npm name (ADR-0003), under the rpc subsystem directory', () => {
    expect(pkg.name).toBe('@insler/rpc-website');
...
describe('content scaffold', () => {
  ...
  test('the landing page opens with the 0-to-value story', async () => {
    ...
    // One install, working in-process service: the install command plus the
    // contract -> handlers -> host + client over the memory transport story.
    expect(landing).toMatch(/bun add @insler\/rpc\s*$/m);
    for (const symbol of [
      'Contract.create',
      'Host.create',
      'Client.create',
  ...
  test('a getting-started guide exists and builds a working in-process service', async () => {
