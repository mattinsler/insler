...
// Invariants for the path-filtered platform integration workflow
// (subsystem-branding issue 0010, replicating the issue 0005 template and the
// di/serde/service replications): run the platform subsystem's private
// integration suite — toolchain via the shared setup action, build the
// published surface, run the suite — only when the platform directory
// changes, and coexist with the unchanged repo-wide workflows and the release
// workflow's wait-for-other-workflows gate. platform needs no real
// infrastructure (its public surface is static analysis of defineService
// declarations, pure codegen, and an in-process plan/diff engine behind the
// StateProvider seam — real state backends don't exist yet), so like the
// di/serde/service replications the job provisions nothing beyond the
// toolchain.

const repoRoot = new URL('..', import.meta.url).pathname;
const source = await Bun.file(join(repoRoot, '.github/workflows/platform-integration.yml')).text();
...
describe('path filtering', () => {
  test('runs on any PR touching the platform subsystem directory', () => {
    expect(triggers.pull_request.paths).toContain('packages/platform/**');
  });

  test('runs on default-branch pushes touching the platform subsystem directory', () => {
    expect(triggers.push.branches).toEqual(['main']);
    expect(triggers.push.paths).toContain('packages/platform/**');
  });

  test('re-runs when the workflow itself changes', () => {
    expect(triggers.pull_request.paths).toContain('.github/workflows/platform-integration.yml');
    expect(triggers.push.paths).toContain('.github/workflows/platform-integration.yml');
  });
});
...
describe('integration job', () => {
  ...
  test('provisions no infrastructure beyond the toolchain (platform runs in-process)', () => {
    // Real infrastructure only where the subsystem actually needs it
    // (subsystem-branding PRD) — platform compiles declarations in-process
    // and its state backend is the injected StateProvider seam, so no infra
    // appears anywhere in the workflow.
    expect(source).not.toMatch(/nats|redis|postgres|kafka|kubectl|kind-action/i);
  });
  ...
  test('builds the published surface before the suite (consumer-grade imports resolve to dist)', () => {
    ...
    const suiteIndex = runs.findIndex((r) => r.includes('@insler/platform-integration'));
  ...
  test('runs the integration suite via its non-default script', () => {
    const suite = steps.find(
      (s) => typeof s['run'] === 'string' && s['run'].includes('@insler/platform-integration')
    );
...
describe('coexistence with the repo-wide workflows', () => {
  ...
  test('the repo-wide CI matrix is untouched by the integration suite', async () => {
    ...
    expect(ci).not.toContain('platform-integration');
  });
});
