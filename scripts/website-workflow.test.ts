...
// Invariants for the path-filtered website workflow (subsystem-branding issue
// 0001): build the apex site whenever a push/PR touches the site or theme
// (both live under packages/website/ — that filter is exactly the site's own
// content, no unrelated packages; the build is the test). Deploys are split
// into least-privilege jobs: a PR-scoped preview job that posts the
// deployment URL back to the PR as a sticky comment (pull-requests: write
// scoped to that job only), and a production job that only ever deploys from
// the public mirror repo (mattinsler/insler). Both stay inert until the
// Cloudflare credentials exist.

const repoRoot = new URL('..', import.meta.url).pathname;
const source = await Bun.file(join(repoRoot, '.github/workflows/website.yml')).text();
...
describe('path filtering', () => {
  test('triggers are content-scoped: packages/website/ holds exactly the apex site and the shared theme', () => {
    for (const paths of [triggers.pull_request.paths, triggers.push.paths]) {
      expect(paths).toContain('packages/website/**');
      // The filter must not reach into other packages — only the workflow
      // file itself rides along.
      expect(paths.filter((p: string) => p.startsWith('packages/'))).toEqual([
        'packages/website/**',
      ]);
    }
  });

  test('production trigger is default-branch pushes touching the site or theme', () => {
    expect(triggers.push.branches).toEqual(['main']);
    expect(triggers.push.paths).toContain('packages/website/**');
  });

  test('re-runs when the workflow itself changes', () => {
    expect(triggers.pull_request.paths).toContain('.github/workflows/website.yml');
    expect(triggers.push.paths).toContain('.github/workflows/website.yml');
  });
});

describe('build job', () => {
  test('builds the apex site (the build is the test)', () => {
    const build = (jobs['build'].steps as Record<string, any>[]).find(
      (s) => typeof s['run'] === 'string' && s['run'].includes('build')
    );
    expect(build?.['run']).toContain('@insler/website');
  });
  ...
  test('hands the built site to the deploy jobs as an artifact', () => {
    ...
    expect(upload?.['with']?.path).toBe('packages/website/dist');
    for (const deploy of ['preview', 'production']) {
      ...
      expect(download?.['with']?.path).toBe('packages/website/dist');
    }
  });
});
...
describe('deploy jobs', () => {
  ...
  test('both deploys publish the apex site build to its own Cloudflare project', () => {
    for (const step of deploys) {
      expect(step['with']?.command).toContain('packages/website/dist');
      expect(step['with']?.command).toContain('--project-name=insler-dev');
    }
  });
...
describe('sticky preview-URL comment', () => {
  ...
  test('is sticky: finds its hidden marker and updates in place, else creates', () => {
    ...
    expect(script).toContain('<!-- website-preview-url -->');
...
describe('release wait-gate coexistence', () => {
  test('the release gate polls runs by head_sha, so a path-filtered workflow that never ran is absent, not blocking', async () => {
    ...
    expect(release).not.toContain('Website');
