...
// Repo-level invariants for the platform subsystem docs site
// (subsystem-branding issue 0010, replicating the issue 0004 template via the
// di/serde/service replications): a private website package under the
// platform subsystem directory builds an independent Astro/Starlight site at
// platform.insler.dev, consuming the shared project identity from
// @insler/theme (nav path back to the project homepage), with Starlight's
// built-in full-text search enabled and a content scaffold seeded from the
// agent library guides — a landing page opening with the 0-to-value story, a
// getting-started guide, and one reference page per umbrella entrypoint and
// per sibling published package (for platform that includes @insler/cli). The
// invariants derive from the umbrella manifest and the shared project data,
// exactly as the template's do.

const repoRoot = new URL('..', import.meta.url).pathname;
const SITE_DIR = 'packages/platform/platform-website';
...
// The umbrella entrypoints and sibling published packages, derived from the
// umbrella manifest and the subsystem directory so the site's reference
// coverage cannot silently drift from the published surface (the shared
...
describe('workspace membership', () => {
  ...
  test('directory equals unscoped npm name (ADR-0003), under the platform subsystem directory', () => {
    expect(pkg.name).toBe('@insler/platform-website');
...
describe('shared project identity', () => {
  ...
  test('site identity (URL, title, tagline) derives from the shared project data', () => {
    ...
    expect(astroConfig).toContain("'platform'");
    expect(astroConfig).not.toContain('https://platform.insler.dev');
  });
...
describe('content scaffold', () => {
  ...
  test('the landing page opens with the 0-to-value story', async () => {
    ...
    // One install, the pipeline working: a defineService declaration scanned
    // into the desired-state model and compiled into artifacts.
    expect(landing).toMatch(/bun add @insler\/platform\s*$/m);
    for (const symbol of ['defineService(', 'scanFleet(', 'createGenerator(']) {
      expect(landing).toContain(symbol);
    }
  });

  test('a getting-started guide exists and walks declaration → scan → generate → plan/apply', async () => {
  ...
  test('exactly one reference page per umbrella entrypoint and per sibling published package', async () => {
    const titles: string[] = [];
