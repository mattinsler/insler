...
// Invariants for the path-filtered serde website workflow (subsystem-branding
// issue 0008, replicating the issue 0004 template and the di replication):
// build the serde docs site whenever a push/PR touches the site's own content
// — the serde-website package, the shared theme, or the workflow itself (the
// build is the test). Deploys are split into least-privilege jobs: a
// PR-scoped preview job that posts the deployment URL back to the PR as a
// sticky comment (pull-requests: write scoped to that job only), and a
...
const source = await Bun.file(join(repoRoot, '.github/workflows/serde-website.yml')).text();
...
describe('path filtering', () => {
  ...
  test('production trigger is default-branch pushes touching the site content', () => {
    ...
    expect(triggers.push.paths).toContain('packages/serde/serde-website/**');
  });

  test('re-runs when the workflow itself changes', () => {
    expect(triggers.pull_request.paths).toContain('.github/workflows/serde-website.yml');
    expect(triggers.push.paths).toContain('.github/workflows/serde-website.yml');
  });
});

describe('build job', () => {
  test('builds the serde docs site (the build is the test)', () => {
    const build = (jobs['build'].steps as Record<string, any>[]).find(
      (s) => typeof s['run'] === 'string' && s['run'].includes('build')
    );
    expect(build?.['run']).toContain('@insler/serde-website');
  });
  ...
  test('hands the built site to the deploy jobs as an artifact', () => {
    ...
    expect(upload?.['with']?.path).toBe('packages/serde/serde-website/dist');
    for (const deploy of ['preview', 'production']) {
      ...
      expect(download?.['with']?.path).toBe('packages/serde/serde-website/dist');
    }
  });
});
...
describe('deploy jobs', () => {
  ...
  test('both deploys publish the serde site build to its own Cloudflare project', () => {
    for (const step of deploys) {
      expect(step['with']?.command).toContain('packages/serde/serde-website/dist');
      expect(step['with']?.command).toContain('--project-name=serde-insler-dev');
    }
  });
...
describe('sticky preview-URL comment', () => {
  ...
  test('is sticky: finds its hidden marker and updates in place, else creates', () => {
    ...
    expect(script).toContain('<!-- serde-website-preview-url -->');
...
describe('release wait-gate coexistence', () => {
  test('the release gate polls runs by head_sha, so a path-filtered workflow that never ran is absent, not blocking', async () => {
    ...
    expect(release).not.toContain('Serde Website');
