...
// Invariants for the path-filtered rpc website workflow (subsystem-branding
// issue 0004): build the rpc docs site whenever a push/PR touches the site's
// own content — the rpc-website package, the shared theme, or the workflow
// itself (the build is the test). Deploys are split into least-privilege
// jobs: a PR-scoped preview job that posts the deployment URL back to the PR
// as a sticky comment (pull-requests: write scoped to that job only), and a
// production job that only ever deploys from the public mirror repo
// (mattinsler/insler). Both stay inert until the Cloudflare credentials
// exist. Mirrors website-workflow.test.ts — the template the remaining
// subsystem sites (issues 0007-0010) replicate.

const repoRoot = new URL('..', import.meta.url).pathname;
const source = await Bun.file(join(repoRoot, '.github/workflows/rpc-website.yml')).text();
...
describe('path filtering', () => {
  ...
  test('production trigger is default-branch pushes touching the site content', () => {
    ...
    expect(triggers.push.paths).toContain('packages/rpc/rpc-website/**');
  });

  test('re-runs when the workflow itself changes', () => {
    expect(triggers.pull_request.paths).toContain('.github/workflows/rpc-website.yml');
    expect(triggers.push.paths).toContain('.github/workflows/rpc-website.yml');
  });
});

describe('build job', () => {
  test('builds the rpc docs site (the build is the test)', () => {
    const build = (jobs['build'].steps as Record<string, any>[]).find(
      (s) => typeof s['run'] === 'string' && s['run'].includes('build')
    );
    expect(build?.['run']).toContain('@insler/rpc-website');
  });
  ...
  test('hands the built site to the deploy jobs as an artifact', () => {
    ...
    expect(upload?.['with']?.path).toBe('packages/rpc/rpc-website/dist');
    for (const deploy of ['preview', 'production']) {
      ...
      expect(download?.['with']?.path).toBe('packages/rpc/rpc-website/dist');
    }
  });
});
...
describe('deploy jobs', () => {
  ...
  test('both deploys publish the rpc site build to its own Cloudflare project', () => {
    for (const step of deploys) {
      expect(step['with']?.command).toContain('packages/rpc/rpc-website/dist');
      expect(step['with']?.command).toContain('--project-name=rpc-insler-dev');
    }
  });
...
describe('sticky preview-URL comment', () => {
  ...
  test('is sticky: finds its hidden marker and updates in place, else creates', () => {
    ...
    expect(script).toContain('<!-- rpc-website-preview-url -->');
...
describe('release wait-gate coexistence', () => {
  test('the release gate polls runs by head_sha, so a path-filtered workflow that never ran is absent, not blocking', async () => {
    ...
    expect(release).not.toContain('RPC Website');
