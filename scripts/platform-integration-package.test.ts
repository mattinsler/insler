...
// Repo-level invariants for the platform subsystem's private integration
// package (subsystem-branding issue 0010, replicating the issue 0005 template
// via the di/serde/service replications): it joins the workspace as a
// non-publishable package (ignored by the publish pipeline, changesets, the
...
// workspace dependencies only), and the package-boundary contract is enforced
// visibly — an internal (non-public) import fails lint. platform is the
// deployment-intent compiler: its public surface is static analysis of
// defineService declarations (the fleet scanner reads real declaration
// files), pure deterministic codegen, and an in-process plan/diff engine
// whose state backend is an injected StateProvider seam — real K8s/serverless
// backends don't exist yet, and the wire-level seam belongs to
// rpc-integration. So like the di/serde/service templates the suite runs
// entirely in-process against the built public surface; its only "real"
// inputs are declaration fixture files inside the package.

const repoRoot = new URL('..', import.meta.url).pathname;
const INTEGRATION_DIR = 'packages/platform/platform-integration';
const pkg = await Bun.file(join(repoRoot, INTEGRATION_DIR, 'package.json')).json();

// platform's public surface: the three-entrypoint umbrella plus @insler/cli,
// the sibling published package — derived from the subsystem directory so the
// allowlist cannot drift from what is published. A consumer of
// @insler/platform necessarily also consumes @insler/service and @insler/rpc
// (the umbrella's own runtime dependencies): the declarations the platform
// compiles are authored with defineService around an rpc contract, so both
// are part of the consumer-grade dependency set.
const surface = await discoverSubsystemSurface(repoRoot, 'platform');
const PUBLIC_PLATFORM_SURFACE = [surface.umbrellaName, ...surface.adapterPackages];
const ALLOWED_INSLER_DEPS = [...PUBLIC_PLATFORM_SURFACE, '@insler/service', '@insler/rpc'];
...
describe('workspace membership', () => {
  ...
  test('directory equals unscoped npm name (ADR-0003), under the platform subsystem directory', () => {
    expect(pkg.name).toBe('@insler/platform-integration');
...
describe('consumer-grade dependencies', () => {
  ...
  test('depends on the umbrella, the CLI sibling, and the declaration stack underneath', () => {
    expect(pkg.dependencies['@insler/platform']).toBe('workspace:*');
    // The CLI is part of the subsystem surface — its consumer-grade entry
    // (the exported run* command API) is exercised by the suite.
    expect(pkg.dependencies['@insler/cli']).toBe('workspace:*');
    // Declarations are authored with defineService around an rpc contract,
    // exactly as a real consumer authors them.
  ...
  test('needs no real infrastructure: no infra dependencies, no server harness', async () => {
    ...
    // (subsystem-branding PRD): platform's public surface is static
    // analysis/compilation — declaration scanning over real fixture files,
    // pure codegen, and an in-process reconciler behind the StateProvider
    // seam — so the suite stays fully in-process. No infra client deps, no
    // process-spawning test harness like the rpc suite's nats-server helper.
    const allDeps = Object.keys({ ...pkg.dependencies, ...pkg.devDependencies });
    for (const name of allDeps) {
      expect(name).not.toMatch(/nats|redis|postgres|kafka|kubernetes/i);
    }
...
describe('package-boundary lint guard', () => {
  ...
  test('a relative import escaping the package fails lint (executable boundary guard)', async () => {
    ...
    await Bun.write(fixture, "import '../../platform/src/index.js';\n");
    try {
