...
// Invariants for the path-filtered service integration workflow
// (subsystem-branding issue 0009, replicating the issue 0005 template and the
// di/serde replications): run the service subsystem's private integration
// suite — toolchain via the shared setup action, build the published surface,
// run the suite — only when the service directory changes, and coexist with
// the unchanged repo-wide workflows and the release workflow's
// wait-for-other-workflows gate. service needs no real infrastructure (its
// public surface is in-process: the transport is an injected parameter, and
// the wire-level NATS seam belongs to rpc-integration), so like the di and
// serde replications the job provisions nothing beyond the toolchain.

const repoRoot = new URL('..', import.meta.url).pathname;
const source = await Bun.file(join(repoRoot, '.github/workflows/service-integration.yml')).text();
...
describe('path filtering', () => {
  test('runs on any PR touching the service subsystem directory', () => {
    expect(triggers.pull_request.paths).toContain('packages/service/**');
  });

  test('runs on default-branch pushes touching the service subsystem directory', () => {
    expect(triggers.push.branches).toEqual(['main']);
    expect(triggers.push.paths).toContain('packages/service/**');
  });

  test('re-runs when the workflow itself changes', () => {
    expect(triggers.pull_request.paths).toContain('.github/workflows/service-integration.yml');
    expect(triggers.push.paths).toContain('.github/workflows/service-integration.yml');
  });
});
...
describe('integration job', () => {
  ...
  test('provisions no infrastructure beyond the toolchain (service runs in-process)', () => {
    // Real infrastructure only where the subsystem actually needs it
    // (subsystem-branding PRD) — service needs none, so no infra appears
  ...
  test('builds the published surface before the suite (consumer-grade imports resolve to dist)', () => {
    ...
    const suiteIndex = runs.findIndex((r) => r.includes('@insler/service-integration'));
  ...
  test('runs the integration suite via its non-default script', () => {
    const suite = steps.find(
      (s) => typeof s['run'] === 'string' && s['run'].includes('@insler/service-integration')
    );
...
describe('coexistence with the repo-wide workflows', () => {
  ...
  test('the repo-wide CI matrix is untouched by the integration suite', async () => {
    ...
    expect(ci).not.toContain('service-integration');
  });
});
