...
// Invariants for the path-filtered platform website workflow
// (subsystem-branding issue 0010, replicating the issue 0004 template and the
// di/serde/service replications): build the platform docs site whenever a
// push/PR touches the site's own content — the platform-website package, the
// shared theme, or the workflow itself (the build is the test). Deploys are
// split into least-privilege jobs: a PR-scoped preview job that posts the
...
const source = await Bun.file(join(repoRoot, '.github/workflows/platform-website.yml')).text();
...
describe('path filtering', () => {
  ...
  test('production trigger is default-branch pushes touching the site content', () => {
    ...
    expect(triggers.push.paths).toContain('packages/platform/platform-website/**');
  });

  test('re-runs when the workflow itself changes', () => {
    expect(triggers.pull_request.paths).toContain('.github/workflows/platform-website.yml');
    expect(triggers.push.paths).toContain('.github/workflows/platform-website.yml');
  });
});

describe('build job', () => {
  test('builds the platform docs site (the build is the test)', () => {
    const build = (jobs['build'].steps as Record<string, any>[]).find(
      (s) => typeof s['run'] === 'string' && s['run'].includes('build')
    );
    expect(build?.['run']).toContain('@insler/platform-website');
  });
  ...
  test('hands the built site to the deploy jobs as an artifact', () => {
    ...
    expect(upload?.['with']?.path).toBe('packages/platform/platform-website/dist');
    for (const deploy of ['preview', 'production']) {
      ...
      expect(download?.['with']?.path).toBe('packages/platform/platform-website/dist');
    }
  });
});
...
describe('deploy jobs', () => {
  ...
  test('both deploys publish the platform site build to its own Cloudflare project', () => {
    for (const step of deploys) {
      expect(step['with']?.command).toContain('packages/platform/platform-website/dist');
      expect(step['with']?.command).toContain('--project-name=platform-insler-dev');
    }
  });
...
describe('sticky preview-URL comment', () => {
  ...
  test('is sticky: finds its hidden marker and updates in place, else creates', () => {
    ...
    expect(script).toContain('<!-- platform-website-preview-url -->');
...
describe('release wait-gate coexistence', () => {
  test('the release gate polls runs by head_sha, so a path-filtered workflow that never ran is absent, not blocking', async () => {
    ...
    expect(release).not.toContain('Platform Website');
