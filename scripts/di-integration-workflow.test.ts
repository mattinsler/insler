...
// Invariants for the path-filtered di integration workflow
// (subsystem-branding issue 0007, replicating the issue 0005 template): run
// the di subsystem's private integration suite — toolchain via the shared
// setup action, build the published surface, run the suite — only when the
// di directory changes, and coexist with the unchanged repo-wide workflows
// and the release workflow's wait-for-other-workflows gate. di needs no real
// infrastructure (it is an in-process dependency-injection container), so
// unlike the rpc template the job provisions nothing beyond the toolchain.

const repoRoot = new URL('..', import.meta.url).pathname;
const source = await Bun.file(join(repoRoot, '.github/workflows/di-integration.yml')).text();
...
describe('path filtering', () => {
  test('runs on any PR touching the di subsystem directory', () => {
    expect(triggers.pull_request.paths).toContain('packages/di/**');
  });

  test('runs on default-branch pushes touching the di subsystem directory', () => {
    expect(triggers.push.branches).toEqual(['main']);
    expect(triggers.push.paths).toContain('packages/di/**');
  });

  test('re-runs when the workflow itself changes', () => {
    expect(triggers.pull_request.paths).toContain('.github/workflows/di-integration.yml');
    expect(triggers.push.paths).toContain('.github/workflows/di-integration.yml');
  });
});
...
describe('integration job', () => {
  ...
  test('provisions no infrastructure beyond the toolchain (di runs in-process)', () => {
    // Real infrastructure only where the subsystem actually needs it
    // (subsystem-branding PRD) — di needs none, so no infra appears anywhere
    // in the workflow.
  ...
  test('builds the published surface before the suite (consumer-grade imports resolve to dist)', () => {
    ...
    const suiteIndex = runs.findIndex((r) => r.includes('@insler/di-integration'));
  ...
  test('runs the integration suite via its non-default script', () => {
    const suite = steps.find(
      (s) => typeof s['run'] === 'string' && s['run'].includes('@insler/di-integration')
    );
...
describe('coexistence with the repo-wide workflows', () => {
  ...
  test('the repo-wide CI matrix is untouched by the integration suite', async () => {
    ...
    expect(ci).not.toContain('di-integration');
  });
});
